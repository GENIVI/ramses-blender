...
class ExportMultipleLayersAndCollections_AllEnabledTest(ExporterTestBase, unittest.TestCase):
    ...
    def test_export_multiple_layers_and_collections_all_enabled(self):
        for exportable_scene in self.get_exportable_scenes_for_test(take_screenshot=True):
            ...
            self.assertTrue(len(layers[0].children), 3) # 2 collections and a cube
            self.assertTrue(isinstance(layers[0].children[0], ramses_export.intermediary_representation.LayerCollectionNode))
            self.assertTrue(isinstance(layers[0].children[1], ramses_export.intermediary_representation.LayerCollectionNode))
            self.assertTrue(isinstance(layers[0].children[2], ramses_export.intermediary_representation.MeshNode))

            self.assertTrue(len(layers[0].children[0].children), 3)  # Cube, Light, Camera
            self.assertTrue(len(layers[0].children[1].children), 2)  # Collection2 1, Cube

            self.assertTrue(isinstance(layers[0].children[1].children[0], ramses_export.intermediary_representation.LayerCollectionNode))
            self.assertTrue(len(layers[0].children[1].children[0].children), 1)  # Cube

            #TODO: Implement SceneObjectIterator to check the same structure on the RAMSES side

if __name__ == '__main__':
    suite_1 = unittest.defaultTestLoader.\
            loadTestsFromTestCase(ExportMultipleLayersAndCollections_AllEnabledTest)

    all_tests = unittest.TestSuite([suite_1])

    success = unittest.TextTestRunner().run(all_tests).wasSuccessful()
    if not success:
        raise Exception('Test "export multiple layers and collections: all enabled" failed')
