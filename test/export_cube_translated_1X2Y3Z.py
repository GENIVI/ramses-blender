...
class TestExportCubeTranslated1X2Y3Z(ExporterTestBase, unittest.TestCase):
    ...
    def test_translation_1x_2y_3z(self):
...
if __name__ == '__main__':
    suite_1 = unittest.defaultTestLoader.\
            loadTestsFromTestCase(TestExportCubeTranslated1X2Y3Z)

    all_tests = unittest.TestSuite([suite_1])

    success = unittest.TextTestRunner().run(all_tests).wasSuccessful()
    if not success:
        raise Exception('Test "cube translated by 1 on X, 2 on Y and 3 on Z" failed.')
