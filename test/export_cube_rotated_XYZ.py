...
class TestExportCubeRotated_XYZ(ExporterTestBase, unittest.TestCase):
    ...
    def test_rotation_mode_XZY(self):
        ...
        for exportable_scene in exporter.get_exportable_scenes():
            ...
            rotation_y_node = ramses_export.RamsesPython.toNode(rotation_x_node.getParent())
...
if __name__ == '__main__':
    suite_1 = unittest.defaultTestLoader.\
            loadTestsFromTestCase(TestExportCubeRotated_XYZ)

    all_tests = unittest.TestSuite([suite_1])

    success = unittest.TextTestRunner().run(all_tests).wasSuccessful()
    if not success:
        raise Exception('Test "cube rotated with rotation mode XYZ" failed.')
