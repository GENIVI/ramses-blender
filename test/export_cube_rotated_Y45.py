...
class TestExportCubeRotatedY45(ExporterTestBase, unittest.TestCase):
    ...
    def test_rotated_y_45(self):
        ...
        for exportable_scene in self.get_exportable_scenes_for_test():
            ...
            col1 = [cos, 0.0, -sin, 0.0]
            col2 = [0.0, 1.0, 0.0, 0.0]
            col3 = [sin, 0.0, cos, 0.0]
...
if __name__ == '__main__':
    suite_1 = unittest.defaultTestLoader.\
            loadTestsFromTestCase(TestExportCubeRotatedY45)

    all_tests = unittest.TestSuite([suite_1])

    success = unittest.TextTestRunner().run(all_tests).wasSuccessful()
    if not success:
        raise Exception('Test "cube rotated Y by 45 degrees" failed.')
