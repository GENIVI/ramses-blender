...
class ExportCubeWithBevelModifier_NotSetInGUI_Test(ExporterTestBase, unittest.TestCase):
    ...
    def test_export_cube_with_bevel_modifier(self):
        for exportable_scene in self.get_exportable_scenes_for_test(take_screenshot=True, evaluate=False):
...
if __name__ == '__main__':
    suite_1 = unittest.defaultTestLoader.\
            loadTestsFromTestCase(ExportCubeWithBevelModifier_NotSetInGUI_Test)

    all_tests = unittest.TestSuite([suite_1])

    success = unittest.TextTestRunner().run(all_tests).wasSuccessful()
    if not success:
        raise Exception('Test "export cube with bevel modifier not set in GUI" failed')
