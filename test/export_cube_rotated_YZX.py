...
class TestExportCubeRotated_YZX(ExporterTestBase, unittest.TestCase):
    ...
    def test_rotation_mode_YZX(self):
        ...
        for exportable_scene in self.get_exportable_scenes_for_test():
            ...
            # YZX: ...Y -> Z -> X -> (Scaling) -> Mesh
            rotation_y_node = ramses_export.RamsesPython.toNode(cube_node.getParent().getParent())
            rotation_z_node = ramses_export.RamsesPython.toNode(rotation_y_node.getParent())
            rotation_x_node = ramses_export.RamsesPython.toNode(rotation_z_node.getParent())

            rotation_x = rotation_x_node.getRotation()
            rotation_y = rotation_y_node.getRotation()
            rotation_z = rotation_z_node.getRotation()

            # if the order is off, the assertions will fail

            # NOTE: the assertion below is wrong because this feature is currently broken
            # TODO: Fix this and then fix this test code
            self.assertEqual(rotation_x, [-60.0, 0.0,  0.0], msg=f'Node is: {rotation_x_node.getName()}')
            #self.assertEqual(rotation_x, [-30.0, 0.0,  0.0], msg=f'Node is: {rotation_x_node.getName()}')
            # NOTE: the assertion below is wrong because this feature is currently broken
            # TODO: Fix this and then fix this test code
            self.assertEqual(rotation_y, [0.0,  -30.0, 0.0], msg=f'Node is: {rotation_y_node.getName()}')
...
if __name__ == '__main__':
    suite_1 = unittest.defaultTestLoader.\
            loadTestsFromTestCase(TestExportCubeRotated_YZX)

    all_tests = unittest.TestSuite([suite_1])

    success = unittest.TextTestRunner().run(all_tests).wasSuccessful()
    if not success:
        raise Exception('Test "cube rotated with rotation mode YZX" failed.')
