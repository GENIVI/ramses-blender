...
class ExportCubeWithBevelModifierTest(ExporterTestBase, unittest.TestCase):
    ...
    def test_export_cube_with_bevel_modifier(self):
        for exportable_scene in self.get_exportable_scenes_for_test(take_screenshot=True, evaluate=True):
...
if __name__ == '__main__':
    suite_1 = unittest.defaultTestLoader.\
            loadTestsFromTestCase(ExportCubeWithBevelModifierTest)

    all_tests = unittest.TestSuite([suite_1])

    success = unittest.TextTestRunner().run(all_tests).wasSuccessful()
    if not success:
        raise Exception('Test "export cube with bevel modifier" failed')
