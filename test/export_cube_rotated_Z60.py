...
class TestExportCubeRotatedZ60(ExporterTestBase, unittest.TestCase):
    ...
    def test_rotate_z_60(self):
        ...
        for exportable_scene in exporter.get_exportable_scenes():
            ...
            col1 = [cos, sin, 0.0, 0.0]
            col2 = [-sin, cos, 0.0, 0.0]
...
if __name__ == '__main__':
    suite_1 = unittest.defaultTestLoader.\
            loadTestsFromTestCase(TestExportCubeRotatedZ60)

    all_tests = unittest.TestSuite([suite_1])

    success = unittest.TextTestRunner().run(all_tests).wasSuccessful()
    if not success:
        raise Exception('Test "cube rotated Z by 60 degrees" failed.')
